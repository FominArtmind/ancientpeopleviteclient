<template>
  <CardBody :selected="selected" :suggested="suggested" :rotated="rotated" :hand="location === 'hand'">
    ...
    <div>
      <template v-if="unit?.hunting?.value"><Info :tooltip="`${unit?.title} has hunting ${unit?.hunting.value} (needed for deer, aurochs, mammoth)`" :changed="unit?.hunting.changed"><Icon v-for="_n in unit?.hunting.value" name="mdi:arrow-up-thin"/></Info></template>
      <template v-if="unit?.attack?.value"><Info :tooltip="`${unit?.title} has attack ${unit?.attack.value} (needed in raids, increases raid attack dice throw by ${unit?.attack.value})`" :changed="unit?.attack.changed"><Icon v-for="_n in unit?.attack.value" name="mdi:axe"/></Info></template>
      <template v-if="unit?.cultureExchange?.value"><Info :tooltip="`${unit?.title} has culture exchange ${unit?.cultureExchange.value} (needed in raids, increases raid difference for determining culture gain by ${unit?.cultureExchange.value})`" :changed="unit?.cultureExchange.changed"><Icon v-for="_n in unit?.cultureExchange.value" name="mdi:torch"/></Info></template>
      <template v-if="unit?.defense?.value"><Info :tooltip="`${unit?.title} has defense ${unit?.defense.value} (needed in raids defending, increases raid defense dice throw by ${unit?.defense.value})`" :changed="unit?.defense.changed"><Icon class="font-smaller" v-for="_n in unit?.defense.value" name="mdi:shield"/></Info></template>
      <template v-if="unit?.raidFoodSteal?.value"><Info :tooltip="`${unit?.title} has raid food steal ${unit?.raidFoodSteal.value}`" :changed="unit?.raidFoodSteal.changed"><Icon name="mdi:arrow-right-top"/>{{ unit?.raidFoodSteal.value }}</Info></template>
    ...
    <div v-if="sneak"><Info :tooltip="`${unit?.title} gains ${sneak} additional food for each unit defeated in raid`">Sneak</Info></div>
...
import { inventionChangedUnitCards, selection } from "../composables/state";
...
const unit = computed((): UnitIC => {
  return inventionChangedUnitCards.value.find(value => value.title === props.card.type) as UnitIC;
});
